...
const steps = ['Pet', 'Task', 'Key', 'Launch'] as const
...
export function CreateAgentPage({ creatorHandle, walletAddress, onConnect }: CreateAgentPageProps) {
  ...
  const canLaunch = Boolean(
    config?.factory && config.keyMarketplace && formReady && Boolean(task?.live),
  )
  const canContinue = useMemo(
    () => step === 0 || (step === 1 && Boolean(task)) || (step === 2 && formReady),
    [formReady, step, task],
  )
    ...
    <div className="app-page create-page live-builder-page">
      <section className="app-page-heading create-heading">
        <div>
          ...
          <p>The appearance is cosmetic. The selected task decides the vault asset, pool route, share token, and execution policy.</p>
        </div>
      ...
      <section className="builder-shell live-builder-shell">
        ...
        <div className="builder-content">
          ...
          {step === 1 && (
            <div className="builder-step">
              ...
              <div className="task-picker" role="group" aria-label="Choose type of task">
                {tasks.map((item) => (
                  <button type="button" aria-pressed={taskId === item.id} className={taskId === item.id ? 'active' : ''} onClick={() => setTaskId(item.id)} key={item.id}>
          ...
          {step === 2 && (
            <div className="builder-step">
              <span className="builder-step-number">03 / KEY MARKET</span>
          ...
          {step === 3 && (
            <div className="builder-step review-step">
              <span className="builder-step-number">04 / LAUNCH</span>
        ...
        <aside className="builder-preview live-pet-preview">
          ...
          <div className="preview-card">
            ...
            <h3>{name || pet.name}</h3>
            <p>{creatorHandle}</p>
            <span className="preview-category">{task?.label ?? 'choose task'}</span>
          ...
          <p className="preview-footnote">Pet and task remain separate forever.</p>
        </aside>
      </section>
    </div>
  )
