...
interface IKeyRevenueReceiver {
    function recordKeyMarketplaceRevenue(address key, uint256 grossVolume) external payable;
}

/// @notice Escrowed whole-unit Agent Key market that attributes every settled fee to its exact Key.
contract KeyMarketplaceV2 is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    uint256 private constant BPS = 10_000;
    uint16 public feeBps;
    IKeyRevenueReceiver public revenueRouter;
    ...
    constructor(address initialOwner, IKeyRevenueReceiver revenueRouter_, uint16 feeBps_) Ownable(initialOwner) {
        if (address(revenueRouter_) == address(0) || address(revenueRouter_).code.length == 0) {
    ...
    function buy(uint256 id, uint128 quantity) external payable nonReentrant {
        ...
        _recordFee(listing.key, subtotal, fee);
    ...
    function acceptOffer(uint256 id, uint128 quantity) external nonReentrant {
        ...
        _recordFee(offer.key, subtotal, fee);
        _pay(payable(offer.buyer), refund);
    ...
    /// @dev Router changes remain possible for recovery, but only to deployed code.
    function setRevenueRouter(IKeyRevenueReceiver nextRouter) external onlyOwner {
        if (address(nextRouter) == address(0) || address(nextRouter).code.length == 0) revert ContractRequired();
        revenueRouter = nextRouter;
        emit RevenueRouterSet(address(nextRouter));
    }

    function _checkKey(IERC20 key) private view {
        if (!approvedKeys[address(key)]) revert InvalidKey();
        _checkKeyShape(key);
    }

    function _checkKeyShape(IERC20 key) private view {
        if (address(key).code.length == 0 || IERC20Metadata(address(key)).decimals() != 0) revert InvalidKey();
    }

    function _recordFee(IERC20 key, uint256 grossVolume, uint256 fee) private {
        if (fee == 0) return;
        revenueRouter.recordKeyMarketplaceRevenue{value: fee}(address(key), grossVolume);
    }

    function _pay(address payable receiver, uint256 value) private {
        if (value == 0) return;
        (bool ok,) = receiver.call{value: value}("");
        if (!ok) revert InvalidOrder();
    }
}
