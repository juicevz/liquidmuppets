...
### Optional reward reinvestment

The Revenue Engine now includes **Claim WETH** and **Buy more and stake** choices. The reinvestment implementation is published but, like Agent Bonds, remains unavailable until the verified contracts are deployed and active. This release does not activate staking or change fee splits.

`MuppetAgentBond.claimBuyAndBond` claims one caller-owned position's global and exact-Key WETH, spends only the selected portion of those rewards through the immutable Pons executor, and opens a new fixed-term bond using only the measured purchase output. The app opens one 15,000-MUPPETS unit with one unused, permanently bound Key. It never binds a Key implicitly or spends existing wallet MUPPETS. Unspent claimed WETH and purchased MUPPETS above the new bond amount return to the caller. The original position and its unlock stay unchanged. Claim, purchase and bond revert together on failure; reverted transactions still cost network gas.

Before signing, the wallet sees the selected WETH input, expected and minimum MUPPETS output, new lock, WETH remainder and estimated ETH gas. Quotes use fresh transaction simulation, expire after 120 seconds, default to 1% slippage and allow at most 3%. The contract enforces a nonzero minimum output and a deadline at most five minutes away. The client rechecks wallet, chain, route capability, rewards and Key capacity before submission. No approval transaction is required. A submitted transaction hash is retained locally to prevent duplicate submission from the same form while confirmation is unknown.

If rewards cannot buy a complete unit, or no unused bound Key is available, ordinary WETH claiming remains the alternative. Independent claims and matured withdrawals remain usable when the bond contract is paused. This is an optional holder purchase, separate from the protocol's five-year vested buybacks, and it creates no guaranteed future return.

Reinvestment verification: run `npm run check`, then run `LIQUIDMUPPETS_QA_URL=http://127.0.0.1:5198 npm run qa:reinvestment` against a local Vite server. The browser harness uses explicit synthetic UI fixtures and cannot submit a mainnet transaction. In `contracts/`, run `forge test --match-contract MuppetRewardReinvestmentTest -vv` and `forge test --match-contract MuppetRewardReinvestmentForkTest --fork-url https://rpc.mainnet.chain.robinhood.com -vv`. The fork tests impersonate the public dev address with locally funded reward/Key fixtures and use the real Pons pool. They are simulations, not earned protocol revenue or mainnet transactions.
